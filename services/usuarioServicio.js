...
const crearUsuario = async (usuario)=> {
    ...
    try {
        ...
        usuario.celular = "No establecido";
...
const actualizarDescripcion = async(idUsuario, usuario, email)=> {
    ...
    if(!usuario.celular || !usuario.descripcionExperiencia || !usuario.urlLinkedin ||
        !usuario.urlLogo || !usuario.urlCv) {
        throw new Error("Datos vacios.")
    }

    const usuarioDescDetalle = await usuarioRepositorio.detalle(idUsuario);
    const usuarioReq = await usuarioRepositorio.buscarEmail(email);
    ...
    usuarioDescDetalle.urlLogo = usuario.urlLogo;
    usuarioDescDetalle.urlCv = usuario.urlCv;

    console.log(usuario.urlLogo);
    console.log(usuario.urlCv);
...
const leerMisOfertas = async (email)=> {
...
const leerMiDescripcion = async (email)=> {
...
export default {crearUsuario, leerUsuario, detalleUsuario, actualizarDescripcion,
