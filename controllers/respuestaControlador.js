...
const postRespuesta = async (req, res)=> {
    ...
    try {
        ...
        if(!req.user.error) {
            const respuesta = await respuestaServicio.crearRespuesta(new RespuestaCrearReqModel(req.body));
...
const getDetailRespuesta = async (req, res)=> {

    try {
        if(!req.user.error) {

            const respuesta = await respuestaServicio.detalleRespuesta(req.params.id);
            respuestasHttp.exito(req, res, new RespuestaDatosResModel(respuesta), 200);
        ...
        respuestasHttp.error(req, res, "Error al leer el detalle de la respuesta", err, 400);
    }
}

export default {postRespuesta, getRespuesta, getDetailRespuesta}